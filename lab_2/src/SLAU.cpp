...
std::vector<double> mulMatrixVector(const std::vector<double>& matrix, const std::vector<double>& vec, int size) {
...
void initMatrix(std::vector<double>* matrix, int size) {
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            if (i == j) (*matrix)[i * size + j] = 2;
            else (*matrix)[i * size + j] = 1;
        }
    }
}
...
void findGoodSolve(const std::vector<double>& A, std::vector<double>* X, const std::vector<double>& B, int size) {
